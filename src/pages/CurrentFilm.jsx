        ...
        <div className='current'>
            <div className="current__top">
                <div className="current__top-content">
                    <div className="current__top-descr">
                        ...
                        <p className="current__top-text">{data.overview || 'Izox topilmadi!'}</p>
                        <ul className="current__top-genres">
                            <li><Link to={'/'}>{getYear(data.release_date || data.first_air_date)}</Link></li>
                            {
                                data.genres?.map((genre, index) => <li key={index}><Link to={'/'}>{genre.name}</Link></li>)
                            }
                            <li><Link to={''}>{getRuntime(data.runtime || data.episode_run_time)}</Link></li>
                        </ul>
                        <button onClick={()=>setmodal(true)} className='btn'>Смотерть трейлер</button>
                    </div>
                    <div className="current__top-img">
                        <img src={import.meta.env.VITE_IMG + data.poster_path} alt="" />
                    </div>
                </div>
                <img src={import.meta.env.VITE_IMG_FULL + data.backdrop_path} alt="" className="current__top-banner" />
                <div className="current__top-cast">
                    <h2 className="current__top-names">В главных ролях</h2>
                    <ul className="current__top-list">
                        {
                            data.credits.cast?.filter((item, index) => index < 6).map((cast, index) => <li key={index}><Link to={'/'}><Cast cast={cast} /></Link></li>)
                        }
                    </ul>
                </div>
            </div>
            ...
            <div className="recommendations container">
                ...
                <div className="recommendations__content">
                    {
                        recommendations.filter((item,index)=>index < 4).map((movie,index) => <Link key={index} to={`/watch/${type}/${movie.id}`} className='recommendations__item'>
                            <img src={import.meta.env.VITE_IMG + movie.poster_path} className='recommendations__item-img' alt="" />
                        </Link>)
